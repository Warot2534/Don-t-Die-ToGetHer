...
def new_game():
  ...
  if call == 'y':
    ...
    input("")
    print("Her: I'll come to your aid in four weeks. However, if you don't survive, I may have to find someone else.")
    print(f'I have given you some {green}Tips{reset} to survive there. Good luck!')
    print("\n\n")
    print("Signal lost....")
    time.sleep(1)
    print(f"====={green}Tips{reset}=====")
    print("--Your girlfriend is coming to help you in 4 weeks. So, DONT DIE...")
    print(
        f"\n{red}Be aware of your Hunger level. Your health might decrease if it exceeds {cyan}4{reset}"
    )
    input("Press Enter..")
    print(f"Your hunger level cannot go below {red}zero{reset}. {bold}Don't eat too much{reset}")
    input("Press Enter..")
    print(f"{green}Fighting zombies my earn you some rewards!{reset}")
  else:
    print("What a pity Your girlfriend could have given you some tips to survive! Anyways, you are alone and don't know what to do. Try to surive")
def math_quiz():
  find_opponent = random.random()
  print("You are finding a zombie to be your opponent...")
  zombie_opp = ['Police officer', 'High school student', 'Engineer', 'Physicist', 'Mathematician']
  ...
  if find_opponent < 0.8:
    ...
    time.sleep(1)
    print(f"He's challenging you to a maths minigame answer each one within {bold}4 seconds{reset}")
    input("Press enter")
  else:
    print(f"You didn't find an opponent you wasted your time and energy to find an opponent...{red}Hunger+2.5{reset}")
    player['hunger']+= 2
  ...
  if zombie_maths > correct_answers:
    ...
    print(f"You answered {correct_answers} questions correctly you lost {(zombie_maths - correct_answers)*3} coins!")
    print("You lost the math quiz! Better luck next time!")
  elif zombie_maths == correct_answers:
    if player['name'] == 'Joe':
      print("Score + 1")
    print(f"You got {correct_answers} questions right! So did the zombie! You earn no coins at all")
  elif zombie_maths < correct_answers:
    if player['name'] == 'Joe':
      print("Score + 1")
    print(f"You are better than a {zombie_opp_ran} with a half of his brain. You got {correct_answers} questions right and the zombie got {zombie_maths}! You won {(correct_answers - zombie_maths)*3} coins!")
...
weapons_list = ["fists", "baseball bat", "knife", "machete", "shotgun","assault rifle"]
armor_names = [
    "none",
    "Leather Jacket",
    "Cloth Robe",
...
def display_status():
  ...
  if secret_location == new_location:
    ...
    if random_coins_events == 1:
      print(f"During the trip to the {str_location}, you encountered a dying man who gave you some coins before he passed away")
    elif random_coins_events ==2:
      print(f"While exploring a dilapidated building on your way to the {str_location}, you stumble upon a hidden stash of coins hidden in a dusty old chest.")
    elif random_coins_events == 3:
      print(f"As you make your way to the {str_location}, you notice an abandoned vehicle by the roadside. Upon searching it, you find some coins hidden in the glove compartment.")
    elif random_coins_events == 4:
      print(f"While traveling to the {str_location}, you find a weathered map with cryptic markings. Upon deciphering it and following the clues, you unearth a buried treasure chest filled with coins.")
    elif random_coins_events == 5:
      print(f"On your journey to the {str_location}, you come across a survivor in distress. After helping them fend off some zombies, they express their gratitude by giving you a handful of coins.")
    time.sleep(2)
    earn_coins()
...
def buy_armor():
  armors = {
      f"{gold}Leather Jacket": ("Leather Jacket", 10, "A basic leather jacket that provides some protection."),
      "Cloth Robe": ("Cloth Robe", 15, "A simple cloth robe that doesn't offer much protection."),
      "Leather Armor": ("Leather Armor", 17, "Lightweight leather armor for agility."),
      "Iron Armor": ("Iron Armor", 20, "A sturdy iron armor that offers good protection."),
      "Chainmail": ("Chainmail", 30, "A chainmail suit that provides decent defense."),
      "Plate Armor": ("Plate Armor", 40, "Heavy plate armor for maximum protection."),
      "Kevlar Vest": ("Kevlar Vest", 50, f"Kevlar vest for excellent protection.{reset}")
  }
  ...
  choice = input("Enter the name of the armor you want to buy or upgrade: ").strip().title()
  if choice in armors:
      ...
      if current_armor == 'none' or armors[current_armor][1] >= armors[choice][1]:
          ...
          print(f"You are upgrading your armor by only paying the prie difference! {current_armor}. Do you want to upgrade? (y/n): ")
          ...
          if upgrade_choice.lower() == 'y':
              ...
              if player['Coins'] >= price_difference:
                  ...
                  player['inventory']['armor'] = choice
                  print(f"{green}You upgraded to a {choice} by paying the price difference of {price_difference} coins.{reset}")
              else:
...
def buy_weapon():
  weapons = {
      "baseball bat": ("baseball bat", 15, "A sturdy baseball bat that packs a punch."),
      "knife": ("knife", 20, "A really sharp knife"),
      "machete": ("machete", 25, "A good weapon to kill someone"),
      "shotgun": ("shotgun", 30, "A powerful shotgun that deals massive damage."),
  ...
  choice = input("Enter the name of the weapon you want to buy or upgrade: ").lower()
  if choice in weapons:
    current_weapon = player['inventory']['weapon']
    if weapons_list.index(current_weapon) >= weapons_list.index(choice) or current_weapon =='fists':
      ...
      print(f"You are upgrading your weapon by paying the price difference from: {current_weapon}. Do you want to upgrade? (y/n): ")
      upgrade_choice = input()
      if upgrade_choice.lower() == 'y':
...
def fight_zombies():
  ...
  damage_reduction_weapon = random.randint((weapon_rank+1)*4,((weapon_rank+3)*4))
  if weapon_rank == 0:
    damage_reduction_weapon = 0
  base_damage = random.randint(5,55)

  damage_reduction_armor = random.randint((weapon_rank+3)*3,((weapon_rank+5)*3))
  if armor_rank == 0:
...
def get_reward(reward):
  ...
  if reward == "food":
    ...
    if weapons_list.index(new_weapon) > weapons_list.index(
      ...
      print("You found...",(new_weapon),"but it's not any better than your current weapon")
  elif reward == "armor":
    armor_found = random.choice(armor_names)
    if armor_names.index(armor_found) > armor_names.index(  player['inventory']['armor']):
      ...
      print("The armor you found is not any better than what you have already...")
...
def find_weapon():
  new_weapon = random.choice(weapons_list)
  if weapons_list.index(new_weapon) > weapons_list.index(player['inventory']['weapon']):
    player['inventory']['weapon'] = new_weapon
    print(f"You find a {new_weapon}. It's better than your current weapon.")
  elif new_weapon =='fists':
...
def escape_zombies():
  ...
  if escape_chance > 2:
    ...
    if player['name'] == 'Mark':
      print("Luck is really not on your side. You are unable to escape and get surrounded by zombies... but because Mark is a Muay Thai fighter he managed to get out with a half of his health!")
      player['health'] /= 2
    else:
      print(
...
def find_cache():
  ...
  player['inventory']['food'] += food_found
  print(
      f"You find {food_found} food items and gain {aid_parts} health."
  )
...
def eat_food():
  if player['inventory']['food'] > 0:
    while True:
      try:
        amount_to_eat = int(input("Enter the amount of food to eat (0 to cancel): "))
        if amount_to_eat < 0:
            ...
            print(f"You ate {amount_to_eat} food and feel less hungry. You also gained {2 * amount_to_eat} health.")
            break
        else:
...
f= open ('title.text','r')
print(''.join(list(f)))
print(f"Welcome to {red}Don't Die ToGetHer{reset} by {magenta}Warot Komontree{reset}")
...
time.sleep(3)
print("You earned a Bravery medal from The Great Leaders around the world...well done",player['name'])
print("🎖️")